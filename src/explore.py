...
def viz_model_preds_no_mlt(version,
    ...
    # Create a new GridSpec for the first row to split it into two equal subplots
    gs1 = mpl.gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=gs[0, :], wspace=0.0)
    ...
    with torch.no_grad():
        for batchi, (imgs, rots, trans, intrins, post_rots, post_trans, binimgs) in enumerate(loader):
            # copy imgs to input_imgs without reference
            input_imgs = imgs.clone()
            if drop:
                # the blackout image red tinted
                input_imgs[:, drop_indexs, :, :, :] *= 0.5
                imgs[:, drop_indexs, :, :, :] = 0
            start = time.time()
            out = model(imgs.to(device),
                    rots.to(device),
            ...
            for si in range(imgs.shape[0]):
                ...
                if save_output:
                        ...
                        # save image from cv2 processing
                        
                        # plt.imsave(f'visualization/blob/blob{batchi:06}_{si:03}.jpg', blob)
                        plt.clf()
                        for imgi, img in enumerate(input_imgs[si]):
                            ax = plt.subplot(gs[1 + imgi // 3, imgi % 3])
                            showimg = denormalize_img(img)
                            # flip the bottom images
                            if imgi > 2:
                            ...
                            plt.annotate(
                                ...
                                bbox=dict(
                                    facecolor='red' if (imgi  in drop_indexs) and drop  else 'black', 
                                    edgecolor='none', 
                        ...
                        plt.legend(handles=[
                            ...
                            mpatches.Patch(color='#76b900', label='Ego Vehicle'),
                            mpatches.Patch(color=(1.00, 0.50, 0.31, 0.8), label='Map (for visualization purposes only)'),
                            mpatches.Patch(color=(1.00, 0.0, 0.0, 0.8), label='Blob Detected Keypoints')
                        ], loc=(0.01, 0.80))
                        plt.imshow(out[si].squeeze(0), vmin=0, vmax=1, cmap='Blues')
                        ...
                        plt.setp(ax2.spines.values(), color='r', linewidth=1)
                        plt.imshow(blob)
